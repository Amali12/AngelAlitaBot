            ...
            if URL_MODE is True:
                if query.from_user.id in ADMINS:
                    btn = [
                        [
                            InlineKeyboardButton(
                                text = "[{}] {}".format(get_size(file.file_size), re.sub('<.*?>|\[.*?\]|\@\w+', '', file.file_name)), callback_data=f'files#{file.file_id}'
                            ),
                        ]
                        for file in files
                    ]
                    ...
                    btn = [
                        [
                            InlineKeyboardButton(
                                text = "[{}] {}".format(get_size(file.file_size), re.sub('<.*?>|\[.*?\]|\@\w+', '', file.file_name)), 
    ...
    if query.data.startswith("file"):
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT ISSUE 🐞", url=f"https://telegram.me/okflixofficial")
            ]]
            btn_lzdv = [
                [
...
async def auto_filter(client, msg, spoll=False):
            ...
            if URL_MODE is True:
                if message.from_user.id in ADMINS:
                    ...
                    btn = [
                        [
                            InlineKeyboardButton(
                                text = "[{}] {}".format(get_size(file.file_size), re.sub('<.*?>|\[.*?\]|\@\w+', '', file.file_name)), callback_data=f'{pre}#{file.file_id}'
                            ),
                        ]
                        for file in files
                        ]
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ 2 ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/OkflixOfficial/21'),
        ] 
    )
    if offset != "":
